...
class ScoreRepositoryImplTest {
    ...
    @DisplayName("저장소에서 findAll 메서드를 호출하면 " +
            "리스트가 반환되고, 해당리스트에는 성적정보가 3개 들어있어야 한다.")
    void findAllTest() {
        // GWT 패턴
    ...
    void findOneTest() {
        ...
        assertEquals("춘시기", score.getName());
    }
    ...
    @DisplayName("새로운 성적정보를 save를 통해 추가하면" +
            "목록의 개수가 4개여야 한다.")
    void saveTest() {
        Score s4 = new Score("박박이", 77, 65, 77, 4, 0, 0.0, Grade.F);

        boolean save = repository.save(s4);
