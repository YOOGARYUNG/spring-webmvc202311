...
/*
    # 컨트롤러
    - 클라이언트의 요청을 받아서 처리하고 응답을 제공하는 개체

    # 요청 URL Endpoint
    1. 학생이 성적정보 등록폼 화면을 보여주고 동시에
       지금까지 저장되어 있는 성적 정보 목록을 조회
    - /score/list   :   GET
...
//    }
    ...
    // - 저장된 성적정보 리스트를 어떠헥 가져오느냐 from 데이터베이스
    @GetMapping("/list")
    ...
    public String register(ScoreRequestDTO score) {
        ...
        /*
            ...
            - 이다이렉트는 요청후에 자동응답이 나가고
              2번째 자동요청이 들어오면서 2번째 응답을 내보냄
            - 따라서 2번째 요청의 URL로 자동 변경됨
         */
        // forward할때는 포워딩할 파일의 경로를 적는것
        // ex) /WEB_INF/views/chap04/score-list.jsp

        //redirect할때는 리다리엑트 요청 URL을 적는것
