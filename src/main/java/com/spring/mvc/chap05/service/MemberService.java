...
public class MemberService {
    ...
    private final PasswordEncoder endoder;

    // 회원가입 처리 서비스
    public boolean join(SignUpRequestDTO dto) {

        // 클라이언트가 보낸 회원가입 데이터를
        // 패스워드 인코딩하여 엔터티로 변화해서 전달

        return memberMapper.save(dto.toEntity(endoder));
    }
    ...
    public LoginResult authenticate(
        ...
        if (!endoder.matches(inputpassword, realPassword)) {
        ...
        if (dto.isAutoLogin()) {
            ...
            // 4. DB에도 쿠키에 관려된 값들(랜덤 세션키, 만료시간)을 갱신
    ...
    public void maintainloginState(HttpSession session, String account) {
        ...
        // 세션으니 수명은 설정한 수명시간에 영향을 받고 브라우저의 수명과 함께한다.
